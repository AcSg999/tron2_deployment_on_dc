...
Matplotlib is included for offline [calibration visual reports](calibration_visualization.md). Generate these from saved images and calibration files with `calibration-report`; the HTML/PNG reports need no ROS connection, GPU service or running frontend. Calibration collection, solving and application remain CLI operations.
