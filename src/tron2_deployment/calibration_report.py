...
</style></head><body><main><header><p class="eyebrow">TRON2 · Offline calibration review / 离线标定检查</p><h1>{escape(title)}</h1><span class="badge">Report review: UNVERIFIED / 报告检查状态：未验证</span><p>This report visualizes existing results. It does not change calibration verification, profiles, or execution settings.<br>此报告仅可视化已有结果，不更改标定验证状态、配置或执行设置。</p>{missing}<p><a href="metrics.json" download>Download metrics JSON / 下载指标</a> · Images are embedded; this HTML opens offline. / 图片已嵌入，可离线打开此 HTML。</p></header><div class="cards">{card_html}</div>{''.join(sections)}<footer>Inspect corner detections, distortion, sample coverage, coordinate axes and independently measured points before applying a calibration. / 应用标定前，请检查角点检测、畸变、采样覆盖、坐标轴和独立测量的验证点。</footer></main></body></html>'''
