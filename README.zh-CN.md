...
通过 CLI 的 `tron2-deploy calibration-guide` 启动标定。浏览器页面帮助用户看清标定板、采集合格视角、查看样本进度并求解。结果仅显示保存文件、下一步，以及内参 RMS 或手眼样本数；独立验证后，仍通过现有 CLI 命令应用结果。详细离线报告作为可选的排查工具。参见[标定流程](docs/calibration.zh-CN.md)。
